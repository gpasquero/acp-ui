...
- **Tool authorization via URL-mode elicitation.** Bumps `@agentclientprotocol/sdk` to 1.3.0 and adds client-side support for URL-mode *elicitation*, so agents (e.g. GlobAI) can ask you to authorize a tool — such as a 3LO grant — in the middle of a turn. The client advertises the capability, shows a dialog with the authorization link, and continues the turn automatically once you've authorized, with no need to re-send your prompt. Upstream on 0.13.1 replied `METHOD_NOT_FOUND` and the flow stalled. The same PR also migrates the **model picker** to the SDK's new `configOptions` API (the bump removed the old `models` field), so choosing and switching models keeps working. Also submitted upstream as [#20](https://github.com/formulahendry/acp-ui/pull/20).
- **Sign-in / authorization links open on the desktop app.** In the desktop (Tauri) build, `window.open` and `target="_blank"` don't reach the system browser, so the auth and tool-authorization links did nothing there. A host-aware helper now opens them in the system browser on desktop (and a new tab on web). Folded into [#18](https://github.com/formulahendry/acp-ui/pull/18) and [#20](https://github.com/formulahendry/acp-ui/pull/20).
- **Terminal capability (desktop).** Implements the ACP `terminal/*` RPCs so an agent can run commands on your machine and read their output — enabling full local file manipulation (`ls`, `grep`, `rm`, `sed`, `git`, …) beyond text read/write. Desktop-only (needs a local subprocess); the client advertises `clientCapabilities.terminal` and spawns via your login shell for correct `PATH`. ⚠️ This grants the connected agent arbitrary command execution on your machine — use it only with agents you trust. Submitted upstream as [#21](https://github.com/formulahendry/acp-ui/pull/21).
